...
contract LPtoken is IERC20, Ownable {
    ...
    uint256 private __previousTreasuryFee = _TreasuryFee;
    ...
    constructor(
        ...
        require(treasuryFeeBps_ >= 0, "Invalid charity fee");
        ...
        require(
            taxFeeBps_ + liquidityFeeBps_ + treasuryFeeBps_ <= 10**4 / 4,
       ...
       _previousLiquidityFee = _liquidityFee;
        ...
        _TreasuryFee = treasuryFeeBps_;
        __previousTreasuryFee = _TreasuryFee;

        //token to add to lquidity pool
        numTokensSellToAddToLiquidity = totalSupply_.mul(10).div(10**4); // 0.1%
    ...
    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(
            _taxFee + _liquidityFee + _TreasuryFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
    }
    ...
    function _takeCharityFee(uint256 tCharity) private {
        if (tCharity > 0) {
            uint256 currentRate = _getRate();
            uint256 rCharity = tCharity.mul(currentRate);
            _rOwned[_TreasuryAddress] = _rOwned[_TreasuryAddress].add(rCharity);
            if (_isExcluded[_TreasuryAddress])
                _tOwned[_TreasuryAddress] = _tOwned[_TreasuryAddress].add(
                    tCharity
                );
            emit Transfer(_msgSender(), _TreasuryAddress, tCharity);
        }
    }
    ...
    function calculateCharityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        if (_TreasuryAddress == address(0)) return 0;
        return _amount.mul(_TreasuryFee).div(10**4);
    }
    ...
    function restoreAllFee() private {
        ...
        _TreasuryFee = __previousTreasuryFee;
    }
